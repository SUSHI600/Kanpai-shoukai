...
try {
    ...
    if ($cartCount !== false) {
        ...
        $displaySql = $pdo->prepare("select items.id as id, items.name as name, items.category_id as category_id, items.price as price, cart.quantity as quantity from items join cart on item.id = cart.item_id where cart.user_id = ?");
